...
[`ai-rules/change-control-manifest.mdc`](../ai-rules/change-control-manifest.mdc) — execution sequence (10 steps), **Verify gate** (IDENTIFY→RUN→READ), **Attempt ledger on disk** (`vault/projects/<project>/sessions/attempt-*.md`), patch budget, confidence permissions, skill routing.
