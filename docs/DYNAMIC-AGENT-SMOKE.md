...
| 13 | `/workday-init` or "วางแผนวันนี้" with bullet tasks | Routes to WORKDAY skill — writes `vault/workday/YYYY-MM-DD.md`; **no** app patches; no plan in `vault/wiki/` |
| 14 | Any skill with SKILL REPORT (e.g. `/vault-recall`) on a work turn | Reply order: main answer → **SKILL REPORT** sections → optional `Logged →` / `Wiki →` **last** (per `vault-issues.mdc` § Close-out order) |

## Post-L4 behavioral checklist (run after Reload)

Run in order after shipping L4 (scoped globs + close-out + builder manifest links):

1. **#11** — wiki auto-ingest without ask-first
2. **#12** — plan-only precedence (no forced patch)
3. **#13** — WORKDAY routing (not `/builder-feature`)
4. **#14** — close-out order (SKILL REPORT before vault lines)

Record **Y** / **N** / **—** in the pass log below.

## Static preflight (automated)

`./scripts/verify-dynamic-smoke-static.sh` checks that skills/rules **contain** the gates for scenarios 1–14 and 8 (smoke exists). It does **not** replace running prompts in Cursor.
...
| 2026-06-08 | #5b feature plan recall | — | Pending until a plan exists under `vault/workday/plans/` |
| 2026-06-08 | #11 wiki auto-ingest | — | **Run after Reload** — Post-L4 checklist step 1 |
| 2026-06-08 | #12 rule/skill precedence | — | **Run after Reload** — Post-L4 checklist step 2 |
| 2026-06-08 | #13 WORKDAY routing | — | **Run after Reload** — Post-L4 checklist step 3 |
| 2026-06-08 | #14 close-out order | — | **Run after Reload** — Post-L4 checklist step 4 |
